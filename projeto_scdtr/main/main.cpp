...
enum class State : byte {start, send_id_broadcast, wait_for_ids, calibrate, apply_control, calib_end}; // states of the system
...
void loop() {
  bool has_data;
    ...
    cli(); has_data = cf_stream->get( frame ); sei();
      ...
      float k[utils->id_ctr]; //initialize coupling gains vector
